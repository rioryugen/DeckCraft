...
COPY scripts/sync-export-engine.cjs /app/scripts/sync-export-engine.cjs
# Bundled export still loads @img/sharp-* native addons from node_modules (not inlined).
RUN rm -rf /app/export-engine \
    && node /app/scripts/sync-export-engine.cjs --force \
    && chmod +x /app/export-engine/py/convert-linux-x64 \
    && cd /app/export-engine \
    && npm init -y \
...
ENV APP_DATA_DIRECTORY=/app_data \
    ...
    EXPORT_PACKAGE_ROOT=/app/export-engine \
    EXPORT_RUNTIME_DIR=/app/export-engine \
    BUILT_PYTHON_MODULE_PATH=/app/export-engine/py/convert-linux-x64 \
...
RUN mkdir -p /app/scripts /app/packages/api /app/packages/web
...
COPY --from=fastapi-builder /app/packages/api /app/packages/api
...
COPY --from=assets-builder /app/export-engine /app/export-engine
COPY --from=assets-builder /app/scripts/sync-export-engine.cjs /app/scripts/sync-export-engine.cjs

COPY --from=nextjs-builder /app/packages/web/.next-build/standalone/ /app/packages/web/
COPY --from=nextjs-builder /app/packages/web/public /app/packages/web/public
COPY --from=nextjs-builder /app/packages/web/.next-build/static /app/packages/web/.next-build/static
