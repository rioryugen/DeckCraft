...
const appDirectoryName = "DeckCraft Open Source";
...
function getAppDataBaseDir(tempRoot: string): string {
  const home = getHomeDir();
  const fallback = path.join(tempRoot, "DeckCraft-app-data");
...
function setElectronPath(name: string, dir: string): void {
  try {
    ...
    console.warn(`[DeckCraft] Failed to set Electron path ${name}=${dir}`, error);
  }
}
...
function appendDiskCacheSwitch(cacheDir: string): void {
  try {
    ...
    console.warn("[DeckCraft] Failed to configure Chromium disk cache path", error);
  }
}
...
export function initializeAppPaths(): ElectronAppPaths {
  ...
  const userDataDir = firstWritableDirectory(
    unique([
      ...
      path.join(tempRoot, "DeckCraft-user-data"),
    ]),
    "user data"
  );
  ...
  const tempDir = firstWritableDirectory(
    unique([path.join(tempRoot, "DeckCraft"), path.join(userDataDir, "temp")]),
    "temporary"
  );
