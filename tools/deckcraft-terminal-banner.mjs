...
/**
 * Docker / startup terminal banner for DeckCraft.
...
function loadAsciiBanner() {
  ...
  const asciiPath = path.join(thisDir, "DeckCraft-ascii.txt");
 ...
 */
export function printDeckCraftStartupBanner(opts = {}) {
  const nextPort = opts.nextPort ?? 3000;
  const fastapiPort = opts.fastapiPort ?? 8000;
  const hostHttpPort =
    opts.hostHttpPort ??
    process.env.DeckCraft_HOST_HTTP_PORT ??
    process.env.DeckCraft_PUBLIC_PORT ??
    "5000";
  ...
  const title = [
    ...
    "   " +
      ...
      BOLD +
      fgRgb(224, 218, 255, "https://github.com/DeckCraft/DeckCraft"),
    muted("   ─────────────────────────────────────────────────────────"),
    "",
  ].join("\n");
  ...
  const summary = [
    ...
    "",
    "   " + muted("Made with ❤️  by the DeckCraft team"),
  ].join("\n");
