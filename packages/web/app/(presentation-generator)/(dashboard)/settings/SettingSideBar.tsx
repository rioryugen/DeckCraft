...
const SettingSideBar = ({ mode, setMode, selectedProvider, setSelectedProvider }: { mode: 'nanobanana' | 'DeckCraft', setMode: (mode: 'nanobanana' | 'DeckCraft') => void, selectedProvider: SettingsSection, setSelectedProvider: (provider: SettingsSection) => void }) => {
            ...
            <div className='mt-6 flex-1'>
                ...
                <div className='p-0.5 rounded-[40px] bg-[#ffffff] w-fit border border-[#EDEEEF] flex items-center justify-center mb-[34px] '>
                    <button className='px-3 font-syne h-[26px] text-[10px] font-medium text-[#3A3A3A] rounded-[70px]'
                        onClick={() => setMode('DeckCraft')}
                        style={{
                            background: mode === 'DeckCraft' ? '#F4F3FF' : 'transparent',
                            color: mode === 'DeckCraft' ? '#5146E5' : '#3A3A3A'
                        }}
                    >Template Based
                    </button>
                ...
                {mode === 'DeckCraft' && <div className='space-y-2.5'>
