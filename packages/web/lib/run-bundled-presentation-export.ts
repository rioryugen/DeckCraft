...
export function getDeckCraftAppRoot(): string {
  return (
    process.env.DeckCraft_APP_ROOT?.trim() ||
...
function extractSessionTokenFromCookieHeader(cookieHeader?: string): string | undefined {
  ...
  const match = cookieHeader.match(/(?:^|;\s*)DeckCraft_session=([^;]+)/);
  if (!match?.[1]) {
    return undefined;
  }
...
async function runBundledPresentationExportLocked(params: {
  ...
  const appRoot = getDeckCraftAppRoot();
  ...
  const tempBase =
    process.env.TEMP_DIRECTORY?.trim() || path.join(os.tmpdir(), "DeckCraft");
