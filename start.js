...
import { printDeckCraftStartupBanner } from "./scripts/DeckCraft-terminal-banner.mjs";
...
const fastapiDir = join(__dirname, "packages/api");
const nextjsDir = join(__dirname, "packages/web");
const nextjsStandaloneServer = join(nextjsDir, "server.js");
const exportSyncScript = join(__dirname, "scripts/sync-export-engine.cjs");
...
const ensurePresentationExportRuntime = async () => {
  ...
  if (!existsSync(exportSyncScript)) {
    console.warn("export-engine sync script not found; skipping runtime check");
    return;
  }
  ...
  try {
    ...
    if (!isDev) {
      throw new Error(
        "export-engine runtime is missing in this container image. Rebuild the image so the runtime package is installed."
      );
    }

    console.warn("export-engine runtime missing in dev mount. Syncing runtime package...");
